...
export function Footer() {
  return (
    <Container className="py-10 px-5 lg:px-10 mb-5 flex flex-col bg-orange-50 rounded-lg font-ubuntu">
      <div className="flex flex-col lg:flex-row my-10 gap-10">
        <div className="w-full lg:w-4/12  flex flex-col gap-2">
          <h1 className="text-2xl lg:text-3xl font-semibold text-heading ">
        ...
        <div className="w-full lg:w-8/12 flex flex-col gap-5 lg:gap-0 lg:flex-row mt-10 lg:mt-0">
          {footerDatas.map((footerdata, index) => (
            <div className="w-full lg:w-1/4 px-0 lg:px-5 flex flex-col gap-1" key={index}>
              ...
              <div className="flex flex-col gap-1">
                {footerdata.data.map((data, idx) => (
                  <a
                    href={data.href}
                    key={idx}
                    className="text-sm lg:text-base  text-gray-800 hover:text-gray-950 transition-all delay-150"
                  >
                    {data.title}
                  </a>
                ))}
          ...
          <div className="w-1/4 px-0 lg:px-5 flex flex-col gap-2 lg:gap-5">
            <h1 className="text-xl font-semibold text-heading">Connect</h1>
            {socialData.map((data, idx) => (
              <a href={data.href} className="flex gap-2 items-center" key={idx}>
                <div className="">{data.icon}</div>
                <p className=" text-gray-800 hover:text-gray-950 transition-all delay-150">
                  {data.title}
                </p>
              </a>
            ))}
...
const FacebookIcon = () => (
  <svg
    ...
    className="size-5  fill-orange-800"
  >
